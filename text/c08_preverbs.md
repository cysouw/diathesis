...
Verbs with preverbs frequently grammaticalise into a more specific meaning. For example, the verb *graben* 'to dig' and *begraben* 'to bury' are still semantically related and show a symmetrical applicative diathesis (see [@sec:preverb-applicative-in-antipassive]). However the meaning of the prefixed verb *begraben* has become semantically restricted to the digging of a grave [@next]. As a general rule, it seems to be the preverbal variant that shows more semantic drift.
...
It is common that such diachronic developments lead to pairs of verbs that semantically are not related anymore [@next]. In the extreme case, the original root of the preverbal verb does not (anymore) in contemporary German [@nnext].
...
Note that in a few examples an intermediate verb exists. For example, the verb *begrünen* 'to plant greenery' [@next b] is transparently related to the adjective *grün* 'green' [@next a]. Now, the intermediate verb *grünen* 'to become green (of plants)' also exists [@next c]. However, the verbs in this section consists of examples in which such an intermediate non-preverbal verb is not, or only very rarely, attested. And even if the intermediate verb exists (e.g. *breiten, dunkeln, härten, kürzen, mehren, wärmen*), then the semantic relation between the adjective and the preverbal verb is still completely transparent, begging the question whether the intermediate verb is really an intermediate step in the derivation or a separate development.
...
### [ –N | NA ] Preverb nominal reciprocal causative

A special subclass of nouns produce a inherently reciprocal verb when combined with a preverb. It appears that all these verbs are derived from a plural noun. For example, the plural *Brüder* 'brother' is the basis of the verb *verbrüdern* 'to fraternise', i.e. 'to become brothers' [@next]. This verb is inherently reciprocal, cf. ~~reciproca tantum~~ as discussed in [@sec:reflexive-real-reciprocals].
...
The relation between *erben* 'inherit' and *enterben* 'disinherit' is peculiar, because the accusative argument *Schreibtisch* of the verb *erben* in [@next a] cannot be expressed in any way with the prefixed verb *enterben* [@next b].
...
Note that there is another usage of the verb *steigen* in the meaning 'to rise' that behaves differently (see [@sec:prepositions-location-drop]).
...
The verb *schenken* 'to pour' used to be common in a context of pouring a drink [@next a], but this is considered old-fashioned in German. The verb *einschenken* has is taking over this usage [@next b].
...
There are two other highly similar diatheses that likewise add an accusative object when a preverb is added. First, some verbs additionally need a dative reflexive pronoun, like when *arbeiten* 'to work' is turned into *erarbeiten* 'to work for something' [@next a]. These verbs are discussed in detail in [@sec:preverb-reflexive-resultative]. Second, many verbs can express the new object as a prepositional phrase before the diathesis. For example, *klettern auf* 'to climb onto X' is turned into *erklettern* 'to climb X' [@next b]. This applicative diathesis is by far the most frequently attested variant of these three diatheses. It is discussed in detail in [@sec:preverb-applicative].
...
The verbs with the prefix *vor-* typically also take a dative argument (cf. [@sec:preverb-dative-addition-accusative]).
...
Typically, preverbal adjectives lead to causative semantics by adding a new nominative subject (see [@sec:preverb-adjectival-causative]). In contrast, the adjective *lustig* 'funny' is the root of the derived verb *belustigen* 'to amuse' and this verb adds a new accusative object [@next].
...
### [ NP– | NPA ] Preverb accusative addition+preposition

The verb *büßen* 'to pay for something' needs a governed preposition [@next a]. This prepositional phrase is optionally retained when the verb is prefixed, leading to *verbüßen* 'to serve a sentence' [@next b]. Additionally, it obtains an accusative argument in the process.
...
### [ ND– | NDA ] Preverb accusative addition+dative

The verb *vertrauen* 'to trust' [@next a] takes a dative argument. The preverbal *anvertrauen* 'to entrust' [@next b] retains this dative and additionally includes a new accusative argument.
...
This diathesis contrasts with a highly similar diathesis that also introduces an accusative, but without the additional reflexive pronoun, like with *morden* 'to murder' [@next]. This diathesis is discussed in detail in [@sec:preverb-accusative-addition]. A possible avenue to explain this difference is that verbs with reflexive, like *antanzen*, typically have an accusative object that is the result of the action. In contrast, verbs without reflexive, like *ermorden*, typically have an object that is the patient of the action. However, this semantic difference does not seem to hold for all examples.
...
- Diese Haltung will ich mir auch antrinken.^[~~DWDS~~: Die Zeit, 07.01.2018, Nr. 02.]
- Es hatte sich feuerrothe Backen erschlafen.^[~~DWDS~~: Spyri, Johanna: Heidi's Lehr- und Wanderjahre. Gotha, 1880.]
- Sie erwandert sich die Atlantikküste.^[~~DWDS~~: Die Zeit, 24.05.2007, Nr. 22.]
- Der Sportler hat sich den ersten Platz erkämpft.
- Schwergewichtler Witali Klitschko hat sich den Titel des Weltverbandes WBC erboxt.^[~~DWDS~~: Berliner Zeitung, 26.04.2004.]
...
The verbs in this section are intransitive verbs, like *gehen* 'to walk' [@next a], that when prefixes by *entˈ-* obtain a new dative argument, like with *entgehen* 'to evade' [@next b]. This is not very common. More widespread, the prefixation of *entˈ-* induces a dative applicative diathesis, turning a prepositional phrase into a dative argument, as discussed extensively in [@sec:preverb-dative-applicative].
...
### [ NA– | NAD ] Preverb dative addition+accusative {#sec:preverb-dative-addition-accusative}

The verbs in this section are transitive verbs, like *lesen* 'to read' [@next a]. When used with a preverb they obtain an additional dative argument, like with *vorlesen* 'to read to somebody' [@next b]. Most prominently, the prefix *vor-* quite productively produces ditransitive verbs with accusative and dative arguments.
...
- ˈein- : *drücken, hämmern, klagen, reiten, schlagen, schießen, treten*
- ˈan-  : *spucken, stampfen*
...
The preposition *durch* only alternates with the preverb *durch-*, either as a verb prefix [@next b] or a verb particle [@next c].
...
- ˈdurch- : *beißen, blättern, bohren, fahren, fließen, laufen, schlagen*
...
There are various still transparent derivations in which there is a rather strong semantic difference [@next].
...
- ˈan-  : *fahren, hüpfen, klagen, springen, tanzen*
- ˈum-  : *fahren, stoßen*
- ˈauf- : *stoßen*
...
- verˈ-  : *fahren (Benzin), heizen, schießen, schlampen, schludern, spekulieren, spielen (Glücksspiel), spritzen, sprühen, zögern*
- erˈ-   : *schwindeln*
- überˈ- : *reden*
...
::: ex
- Der Tunnel führt unter die Bahnstrecke hindurch.
- Der Tunnel unterführt die Bahnstrecke.
:::
...
::: ex
a. Ich bettele um ein Stück Brot.
...
However, the datives in many of these examples can be interpreted as a benefactor, so it might be better to interpret them as a kind of *für* benefactive dative, see [@sec:prepositions-benefactive-dative]. The reflexive marking might also not be necessary, i.e. the action could also be performed in favour of somebody else. When both these possibilities come together, then this diathesis would be a transparent stack of applicative +> benefactive dative +> self-inflicting reflexive [@next] and would not warrant a separate subsection (for the notion 'stack' see [@sec:intro-stacking]).

::: ex
Ich bettele um ein Stück Brot für dich. \
(+> applicative) Ich erbettele ein Stück Brot für dich. \
(+> benefactive dative) Ich erbettele dir ein Stück Brot. \
...
### [ NDP | NDA ] Preverb governed applicative+dative

An unusual variant of a governed applicative is illustrated with the diathesis between *drohen* [@next a] and *androhen* [@next b], both meaning 'to threaten'. Additional to the change from a prepositional phrase to an accusative there is a dative that does not change in this diathesis. Such unchanged datives are highly unusual together with an applicative. Note that the reversal, i.e. a dative applicative with an unchanged accusative, is much more widespread (see [@sec:preverb-dative-applicative-accusative]). This asymmetry is yet another example of the dative-accusative asymmetry as also observed with reflexive pronouns ([@par:reflexive-generalisations]), the drop hierarchy ([@sec:case-object-demotion]) and the antipassive hierarchy ([@sec:prepositions-demotion-of-object]).
...
Some verbs allow for an additional accusative argument alongside a dative applicative diathesis. For example, the verb *flüstern* 'to whisper' [@next a] has an additional accusative argument in the form of reported speech. This argument is retained in the diathesis *zuflüstern* 'to whisper to somebody' [@next b]. The *zu-* diathesis appears to be rather productive with verbs of communication [cf. @wiemer2007: 472].

::: ex
- "Jesses Maria […]", flüsterte ich zu dem Russen […].^[~~DWDS~~: Die Zeit, 29.05.2012, Nr. 19.]
-  "Jesses Maria", flüsterte ich dem Russen zu.
:::
...
The verb *ansehen* has various different meanings, as summarised in [@sec:preverb-dative-reflexive]. One of these can approximately be translated into English as 'to notice' [@nnext]. In this sense of *ansehen*, the possessor of the accusative argument from *sehen* is obligatorily expressed as a dative with *ansehen*.
...
A few examples exist of a preverb diathesis in which a dative argument is changed into an accusative argument. For example, *folgen* 'to follow' [@next a] takes a dative, while *verfolgen* 'to pursue' [@next b] takes an accusative.
...
### [ NA | ND ] Preverb accusative-to-dative

Conversely, the diathesis from *jagen* 'to hunt' [@next a] to *nachjagen* 'to chase' [@next b] changes an accusative argument into a dative argument. This seems to be less frequent than the reverse, as discussed in the previous section.

::: ex
- Die Polizei jagt den Verbrecher.
- Die Polizei jagt dem Verbrecher nach.
:::
...
The alternation between *wundern* 'to amaze' [@next a] and *bewundern* 'to be in awe' [@next b] reverses the nominative and accusative arguments. Note that the verb *wundern* also allows for a reflexive conversive diathesis [@next c], see [@sec:reflexive-preposition-passive], but that construction cannot function as a intermediate step in this diathesis.
